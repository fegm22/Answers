...
public class BinarySearchTree<T extends Comparable<T>> implements Tree<T> {
    ...
    private boolean check(Node<T>  n, T min, T max){
        if(n==null)
            return true;

        if(n.getData().compareTo(min) <= 0 || n.getData().compareTo(max) >= 0)
            return false;
        return check(n.getRightChild(), min, n.getData()) && check(n.getRightChild(), n.getData(), max);
    }
